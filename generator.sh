...
function get_random_name {
    while true; do
        adjective=${left[$((RANDOM % ${#left[@]}))]}
        surname=${right[$((RANDOM % ${#right[@]}))]}
        name="${adjective}_${surname}"
        if [ "$name" != "boring_wozniak" ]; then
            break
        fi
    done
    ...
    retry=$1
    if [ $retry -gt 0 ]; then
        name="${name}$(random_digit)"
    fi

    echo $name
}

random_name=$(get_random_name 1)
#echo $random_name
